...
char jogadorNome[MAX_NAME_LEN] = "Anonimo";
...
// Funcoes de ranking
int carregarRanking(RankEntry lista[], int maxEntradas);
void salvarRanking(RankEntry lista[], int n);
void atualizarRanking(const char *nome, int pontos);
void mostrarRankingTop(int topN);
...
// Limpa o buffer de entrada (stdin) até o fim da linha
void limparBuffer()
{
    int c;
    while ((c = getchar()) != '\n' && c != EOF)
...
// Carrega ranking do arquivo. Retorna numero de entradas lidas.
int carregarRanking(RankEntry lista[], int maxEntradas)
{
    FILE *f = fopen(RANKING_FILE, "r");
    if (!f)
        return 0; // sem arquivo ainda

    int count = 0;
    char linha[256];
    while (fgets(linha, sizeof(linha), f) && count < maxEntradas)
    {
        ...
        *sep = '\0';
        char *nome = linha;
        char *pts_str = sep + 1;
        char *nl = strchr(pts_str, '\n');
        if (nl)
            *nl = '\0';

        strncpy(lista[count].nome, nome, MAX_NAME_LEN - 1);
        lista[count].nome[MAX_NAME_LEN - 1] = '\0';
        lista[count].pontos = atoi(pts_str);
        count++;
    }
    fclose(f);
    return count;
...
// Salva lista completa de ranking 
void salvarRanking(RankEntry lista[], int n)
{
...
// Atualiza o ranking com (nome, pontos).
// Se ja existir nome e o novo pontos for maior, atualiza.
// Senao adiciona nova entrada.
// Mantem ordenacao decrescente por pontos.
void atualizarRanking(const char *nome, int pontos)
{
    RankEntry lista[MAX_PLAYERS];
    int n = carregarRanking(lista, MAX_PLAYERS);

    // procura por nome (case-insensitive)
    int idx = -1;
    for (int i = 0; i < n; ++i)
    {
        if (compararIgnorarCase(lista[i].nome, nome) == 0)
        {
            idx = i;
            break;
        }
    ...
    if (idx != -1)
    {
        if (pontos > lista[idx].pontos)
        {
            lista[idx].pontos = pontos;
        }
        else
        {
            // nao melhora o record -> ainda vamos reordenar/substituir possivel
        }
    }
    else
    {
        if (n < MAX_PLAYERS)
        {
            strncpy(lista[n].nome, nome, MAX_NAME_LEN - 1);
            lista[n].nome[MAX_NAME_LEN - 1] = '\0';
            lista[n].pontos = pontos;
            n++;
        }
        else
        {
            // se lista cheia, substituir se for maior que o menor
            int menor = 0;
            for (int i = 1; i < n; ++i)
                if (lista[i].pontos < lista[menor].pontos)
                    menor = i;
            if (pontos > lista[menor].pontos)
            {
                strncpy(lista[menor].nome, nome, MAX_NAME_LEN - 1);
                lista[menor].nome[MAX_NAME_LEN - 1] = '\0';
                lista[menor].pontos = pontos;
            }
        }
    }
    ...
    salvarRanking(lista, n);
}

// Mostra topN do ranking (se nao existir, mostra o que houver)
void mostrarRankingTop(int topN)
{
    RankEntry lista[MAX_PLAYERS];
    int n = carregarRanking(lista, MAX_PLAYERS);
    ...
    printf("\n--- RANKING GLOBAL (TOP %d) ---\n\n", topN);
    if (topN > n)
        topN = n;
    for (int i = 0; i < topN; ++i)
    {
        printf("%2d. %-25s  %4d pts\n", i + 1, lista[i].nome, lista[i].pontos);
    }
    printf("\n");
}

// Pede o nome do jogador (no inicio do jogo)
void pedirNomeJogador()
{
    limparTela();
    printf("Digite seu nome (max %d chars): ", MAX_NAME_LEN - 1);
    if (fgets(jogadorNome, sizeof(jogadorNome), stdin) != NULL)
    {
        char *nl = strchr(jogadorNome, '\n');
        if (nl)
            *nl = '\0';
        if (strlen(jogadorNome) == 0)
            strncpy(jogadorNome, "Anonimo", sizeof(jogadorNome) - 1);
    }
    else
    {
        strncpy(jogadorNome, "Anonimo", sizeof(jogadorNome) - 1);
...
void introducao()
{
    // resetar variaveis exceto nome do jogador
    resetGame();
...
void eventoCelaBruxa()
{
    ...
    printf("2 - Andar pelas paredes\n");
    printf("3 - Ir direto pelo centro\n");
    printf("Escolha: ");
    if (scanf("%d", &escolha1) != 1)
    {
        limparBuffer();
    ...
    if (escolha1 == 1)
    {
        ...
        printf("2 - Tentar desarmar\n");
        printf("3 - Pisar nelas (arriscado)\n");
        printf("Escolha: ");
        if (scanf("%d", &escolha2) != 1)
        {
            limparBuffer();
...
void eventoDragaoFilhote()
{
    ...
    else
    {
        ...
        printf("Voce nota: ele esta morrendo de fome!\n\n");
        ...
        else if (escolha2 == 2)
        {
            ...
            printf("Comerciante se sente mal: 'Eu... tenho algo pra voce.'\n");
        ...
        else
        {
            ...
            printf("Comerciante: 'Voce e sabio. Pegue isto.'\n");
...
void eventoAnaoEgnar()
{
    ...
    if (escolha1 == 1)
    {
        ...
        printf("'Meu nome e Egnar.'\n\n");
        printf("'Conheci um paladino chamado Damian.'\n");
        printf("'Ele me deu razao para existir.'\n");
    ...
    else if (escolha1 == 2)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                printf("Egnar: 'Tudo bem, a intencao conta.'\n");
                pontosSabedoria += 1;
            }
        }
    ...
    else
    {
        ...
        {
            printf("\nVoce: 'Sim, tenho medo.'\n");
            printf("Egnar sorri: 'Honestidade. Isso e raro.'\n");
            printf("'Damian tinha medo... mas nunca mostrava.'\n");
    ...
    printf("'O proposito nao e servir. E VIVER.'\n");
...
void eventoDrakeAnciao()
{
    ...
    else if (dragaoFilhoteSalvo == 1)
    {
        ...
        printf("Uma lagrima cai do olho do drake.\n");
        printf("Drake: 'Voce nao e como eles. Voce ESCUTA.'\n\n");
...
void eventoGargulaCharadas()
{
    ...
    {
        acertos++;
        printf("\nGargula: 'CORRETO. Pensar é melhor que agir.'\n");
    }
    else
        printf("\nGargula: 'ERRADO. Mas continua...'\n");
    pausar();
    ...
    {
        acertos++;
        printf("\nGargula: 'CORRETO. Amor distorcido ainda é amor.'\n");
    }
    else
        printf("\nGargula: 'ERRADO.'\n");
    pausar();
...
void eventoArmadurasDancarinas()
{
    ...
    if (escolha == 1)
    {
        ...
        printf("'Forcasempre escolhe forca...'\n\n");
...
void calcularRanking()
{
    ...
    // Atualiza arquivo de ranking com o jogador atual
    atualizarRanking(jogadorNome, pontosTotal);

    // Carrega ranking para mostrar posicao
    RankEntry lista[MAX_PLAYERS];
    int n = carregarRanking(lista, MAX_PLAYERS);

    // encontra posicao do jogador
    int pos = -1;
    for (int i = 0; i < n; ++i)
    {
        if (compararIgnorarCase(lista[i].nome, jogadorNome) == 0)
        {
            pos = i + 1; // ranks 1-based
            break;
        }
    }
    ...
    if (pos != -1)
    {
        printf("Parabens, %s! Voce terminou em %d%c lugar com %d pontos.\n\n",
               jogadorNome, pos,
               (pos % 10 == 1 && pos % 100 != 11) ? 'o' : (pos % 10 == 2 && pos % 100 != 12) ? 'o'
                                                                                             : 'o',
               pontosTotal);
    }
    else
...
// Zera variaveis do jogo (para novo playthrough), NAO zera jogadorNome
void resetGame()
{
    pontosCompaixao = 0;
    pontosAstucia = 0;
    ...
    temMithril = 0;
    temEscamaDragao = 0;
    dragaoFilhoteSalvo = 0;
    dragaoFilhoteMorto = 0;
    rotaEscolhida[0] = '\0';
}
